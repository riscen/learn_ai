...
import PerceptronControls from "./controls/PerceptronControls";
...
class PerceptronUI extends Component {
  ...
  canvas2CartesianCoord = pos => {
    ...
    return {
      x: (pos.x - canvas.offsetLeft - canvas.width * 0.5) / this.scale,
      y: -(pos.y - canvas.offsetTop - canvas.height * 0.5) / this.scale
    };
  };
  ...
  cartesian2CanvasCoord = pos => {
    ...
    const x = this.scale * pos.x + canvas.offsetLeft + canvas.width * 0.5;
    const y = -this.scale * pos.y + canvas.offsetTop + canvas.height * 0.5;
    return {
  ...
  handleClick = event => {
    ...
    const inputClass = this.perceptron.trained
      ? this.perceptron.calculateOutput([cartesianPos.x, cartesianPos.y]) === 1
        ? 1
        : 0
      : event.button === 0
      ? 0
  ...
  trainPerceptron = () => {
    ...
    epochsPassed = this.perceptron.train(x, y, learningRate, epochs);
    if (epochsPassed === -1) {
      window.alert(
        "The Perceptron couldn't converge.\nRemember that the Perceptron only can classify linearly separable classes"
      );
    } else {
      this.drawLine();
      this.setState({
        epochsPassed: epochsPassed,
    ...
    return (
      <div className="perceptron">
        ...
        <div className="perceptron-controls-container">
          <PerceptronControls
            disableControls={this.state.perceptronTrained}
            epochs={this.state.epochs}
            epochsPassed={this.state.epochsPassed}
